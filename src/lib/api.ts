  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Only log unexpected errors (5xx), not business logic errors (4xx)
        // Suppress all 400 errors as they are expected business logic validation errors
        // The browser console will still show the network request, but we won't add extra logging
      ...
      // Only log actual network/connection errors, not business logic errors
      // Check if it's a network error (not a response error)
